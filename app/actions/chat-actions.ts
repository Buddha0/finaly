...
export async function sendAssignmentMessage(
  ...
  try {
    ...
    const fileData = fileUrls ? { fileUrls } : {};
...
export async function getAssignmentUnreadCount(assignmentId: string) {
  try {
    ...
    // Count unread messages for this specific assignment
    const count = await prisma.message.count({
      where: {
        assignmentId,
        receiverId: userId,
    ...
    console.error("Failed to get assignment unread count:", error);
    return { 
      success: false, 
      error: "Failed to get assignment unread count",
      count: 0
    };
  }
} 